 ...
 * VisionClaw REST API so it gets broadcast to all WebSocket clients.
 *
 * Usage:
 *   node scripts/memory-flash-bridge.mjs [--visionclaw-url http://localhost:3001]
 *
 * Environment:
 *   PGHOST / PGPORT / PGUSER / PGPASSWORD / PGDATABASE — RuVector PG connection
 *   VISIONCLAW_URL — base URL for VisionClaw API (default: http://localhost:3001)
 */
...
const VISIONCLAW_URL =
  args.find((_, i) => args[i - 1] === '--visionclaw-url') ||
  process.env.VISIONCLAW_URL ||
  'http://localhost:3001';

const ENDPOINT = `${VISIONCLAW_URL}/api/memory-flash`;
const BATCH_ENDPOINT = `${VISIONCLAW_URL}/api/memory-flash/batch`;
