...
//!
//! Domain-safe subset extracted from `webxr::actors::messages::graph_messages`.
//! Webxr-internal messages (`GetGraphStateActor`, `RequestGraphUpdate`) remain in
//! `webxr::src::actors::messages::graph_messages` as shims.
//!
//! ## MessageId note
//! `UpdateNodePositions.correlation_id` uses raw `uuid::Uuid` rather than
//! `webxr::actors::messaging::MessageId` (a newtype wrapper around the same Uuid)
...
use visionflow_domain::models::graph::GraphData as ServiceGraphData;
use visionflow_domain::models::metadata::{FileMetadata, MetadataStore};
use visionflow_domain::models::node::Node;
use visionflow_domain::models::workspace::{
    CreateWorkspaceRequest, UpdateWorkspaceRequest, Workspace, WorkspaceFilter, WorkspaceQuery,
};
use visionflow_domain::types::{BinaryNodeData, Vec3Data};
...
pub struct UpdateNodePositions {
    ...
    /// Uses raw `Uuid` to avoid newtype mismatch with `webxr::actors::messaging::MessageId`.
...
#[derive(Message)]
#[rtype(result = "Result<visionflow_domain::models::workspace::WorkspaceListResponse, String>")]
