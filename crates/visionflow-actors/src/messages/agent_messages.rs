//! Domain-safe agent/swarm message types.
//!
//! Blocked in webxr: `UpdateBotsGraph` (refs `services::bots_client::Agent`),
//! `SetAgentMonitorAddr` (refs `actors::AgentMonitorActor` addr).
//! Everything else depends only on `actix`, `chrono`, `serde`, `serde_json`,
//! `std`, `visionflow_domain` types.
...
use visionflow_domain::types::claude_flow::AgentStatus;
