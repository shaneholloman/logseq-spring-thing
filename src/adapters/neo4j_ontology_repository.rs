...
impl Neo4jOntologyRepository {
    /// Access the underlying Neo4j graph connection.
    ///
    /// Used by `github_sync_service` to execute block-level Cypher generated by
    /// `block_level_parser::blocks_to_neo4j_queries()` (ADR-068 D6).
    pub fn neo4j_graph(&self) -> &Arc<Graph> {
        &self.graph
    }
