...
//!
//! ADR-050 §"Rust schema additions" defines the `Visibility` enum the
//! schema-and-binary sibling agent owns on `KGNode`. Until that lands,
//! this module is the single source of truth for the type and keeps the
//! parser compilable in isolation.

use serde::{Deserialize, Serialize};

/// Two-state visibility for `:KGNode` rows.
///
/// TODO(sibling: schema-and-binary): move this to `src/models/node.rs` once
/// the sibling agent's `KGNode` struct additions (visibility / owner_pubkey
/// / opaque_id / pod_url) land. For now the parser owns the type so it can
/// compile and ship independently.
...
impl Visibility {
    /// Render as the exact string stored on the Neo4j `:KGNode` row.
    pub fn as_neo4j_str(self) -> &'static str {
        match self {
