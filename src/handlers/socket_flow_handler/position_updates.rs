...
pub(crate) fn handle_request_full_snapshot(
    ...
    let fut = async move {
        ...
        let snapshot = crate::actors::messages::PositionSnapshot {
            knowledge_nodes,
            agent_nodes,
    ...
    ctx.spawn(fut.map(move |(snapshot, nta), _act, ctx| {
        ...
        if !all_nodes.is_empty() {
            ...
            let private_hidden_ids =
                crate::actors::client_coordinator_actor::compute_private_opaque_ids(
                    &nta,
...
pub(crate) fn handle_node_drag_update(
    ...
    ctx.spawn(
        actix::fut::wrap_future::<_, SocketFlowServer>(fut).map(|_, act, _ctx| {
